export const translations = {
  uk: {
    stack: {
      badge: 'Технічний стек',
      title: 'Мої навички',
      subtitle: 'Технології розробки та творчі інструменти для створення сучасних веб-додатків і візуального контенту',
      frontend: 'Frontend розробка',
      creative: 'Креативні інструменти',
      stats: {
        years: 'Роки досвіду',
        projects: 'Проєктів', 
        technologies: 'Технологій',
        satisfaction: 'Задоволення'
      }
    },
    nav: {
      home: 'Головна',
      stack: 'Стек',
      experience: 'Досвід',
      projects: 'Проєкти',
      faq: 'FAQ',
      contact: 'Контакти'
    },
    faq: {
      badge: 'Питання',
      title: 'Часті',
      titleHighlight: 'питання',
      subtitle: 'Відповіді на найпоширеніші запитання про співпрацю та технології'
    },
    hero: {
      badge: 'Frontend • React • JavaScript',
      title: 'Створюю',
      titleHighlight: 'швидкі',
      titleEnd: 'та акуратні інтерфейси',
      subtitle: 'React, TypeScript, REST/GraphQL, адаптивна верстка, тестування, CI/CD — все для успішного продукту',
      emailButton: 'Написати листа',
      description: 'Створюю сучасні веб-додатки з React та TypeScript. Поєдную технічні навички з творчим підходом для створення унікальних цифрових рішень.',
      cta: 'Зв\'язатись',
      email: 'olexandr.bushko@example.com'
    }
  },
  en: {
    stack: {
      badge: 'Tech Stack',
      title: 'My Skills',
      subtitle: 'Development technologies and creative tools for building modern web applications and visual content',
      frontend: 'Frontend Development',
      creative: 'Creative Tools',
      stats: {
        years: 'Years of experience',
        projects: 'Projects',
        technologies: 'Technologies', 
        satisfaction: 'Satisfaction'
      }
    },
    nav: {
      home: 'Home',
      stack: 'Stack',
      experience: 'Experience',
      projects: 'Projects',
      faq: 'FAQ',
      contact: 'Contact'
    },
    faq: {
      badge: 'Questions',
      title: 'Frequently Asked',
      titleHighlight: 'Questions',
      subtitle: 'Answers to the most common questions about collaboration and technologies'
    },
    hero: {
      badge: 'Frontend • React • JavaScript',
      title: 'Creating',
      titleHighlight: 'fast',
      titleEnd: 'and clean interfaces',
      subtitle: 'React, TypeScript, REST/GraphQL, responsive design, testing, CI/CD — everything for a successful product',
      emailButton: 'Send email',
      description: 'I create modern web applications with React and TypeScript. I combine technical skills with a creative approach to create unique digital solutions.',
      cta: 'Get in touch',
      email: 'olexandr.bushko@example.com'
    }
  }
}
